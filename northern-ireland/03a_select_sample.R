...
# d_cohort %>% group_by(hh_n_cat) %>% tally()
# d_cohort %>% group_by(health_board) %>% tally()
# 
...
t_sample_selection <- tribble(
    ~step, ~criteria, ~n,
        ...
        5, "has at least 3 doses",
            d_cohort %>% filter(is_c20, has_dob_sex, is_age_gte18, has_lsoa, has_3_doses) %>% nrow(),
