...
d_death <-
    ...
    # make wide
    mutate(
        death_c19_date   = if_else(death_c19_secondary_flg == 1, death_date, NA_Date_),
        death_other_date = if_else(death_c19_secondary_flg == 0, death_date, NA_Date_)
    ) %>%
    select(
        study_id,
        death_c19_date,
...
d_hosp_c19 <-
    d_hosp %>%
    filter(admis_emergency_with_covid19_flg == 1) %>%
    mutate(
...
qsave(
    d_cohort,
    file = ("input/d_cohort_clean_broad.qs")
)
